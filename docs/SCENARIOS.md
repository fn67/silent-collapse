# SCENARIOS.md — All Content, Pillar Mappings, Phrase Pools, Config Values

## Source of Truth

All strings, scenarios, pillar mappings, and phrases in this file must live in `/config/scenarios.js`.
No component may hardcode any scenario text, pillar, or phrase.
...
Each scenario has an `id`, `text`, and `pillar`.
Pillar assignments below are approximate — user will manually reorganize later.
...
Shown randomly when user clicks "Do Nothing" during normal play or recovery.
...
Shown randomly when user clicks "Take Action" during normal play.
...
## Recovery Action Phrases

Shown randomly when user clicks "Take Action" during recovery phase only.
These carry the weight of confronting past silence.
...
1. Add a new object to the `SCENARIOS` array with `id`, `text`, and `pillar`
2. Update `TOTAL_SCENARIOS` count
3. No component changes needed

## Updating Pillar Assignments

To reassign a scenario to a different pillar:
1. Find the scenario by id in the SCENARIOS array
2. Change its `pillar` field to: "leadership" | "innovation" | "credibility" | "togetherness"
3. No component changes needed